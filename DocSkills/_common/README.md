# _common — shared helpers for DocSkills

Not a SKILL. Holds code reused by multiple skill `scripts/run.py` entries.
Skill scripts import from it after putting `DocSkills/` on `sys.path`, e.g.
...
Skills stay portable: nothing here imports from `harness/`.
