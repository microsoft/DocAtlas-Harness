...
_DOC_SKILLS = _THIS.parent.parent.parent          # …/DocAtlas/DocSkills
...
def _build_markdown_pages(
    reader: MarkdownReader, page_ids: list[int], fig_filter: FigureFilter,
...
def _build_text_pages(pdf_path: str, page_ids: list[int]) -> tuple[
...
def _fetch_figures(
    ...
    for page_num, reqs in by_page.items():
        ...
        available_refs = [f"image_{i + 1}" for i in range(n)]
        ...
        for r in reqs:
            ...
            uri = MarkdownReader.encode_image_file(img_path)
            if uri is None:
                errors.append({
                    "page": page_num, "ref": ref,
                    "reason": "unreadable",
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--force-figures", action="store_true",
    ...
    if args.with_image and pdf_exists:
        rendered = render_pages_to_base64(pdf_path, page_ids, zoom=args.zoom)
    ...
    if args.figures and md_reader is not None:
        try:
            ...
            if isinstance(requests, list) and requests:
                figure_images, figure_errors = _fetch_figures(
                    md_reader, requests, fig_filter, args.force_figures,
                )
        except json.JSONDecodeError:
            figure_errors.append({"reason": "invalid_json", "raw": args.figures})
    ...
    if remaining:
        ...
        hint_parts.append(
            ...
            "conclude `Not answerable`, or provide a final answer based on insufficiency."
        )
    if memory_on:
        hint_parts.append(
    ...
    if sess_path:
        try:
            ...
            workspace["read_history"] = read_history
            ...
            session_patch = {
                "workspace.read_history.appended": {"pages": returned},
