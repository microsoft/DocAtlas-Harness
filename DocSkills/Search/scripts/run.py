...
from session_io import load_session, require_session_file, save_session  # type: ignore  # noqa: E402
...
def _filter_tree_by_doc(structure: list | dict, doc_id: str) -> list | None:
    ...
    for n in nodes:
        ...
        if sp:
            if sp == key_l or sp == f"{key_l}.pdf" or sp.startswith(f"{key_l}.") or sp.startswith(f"{key_l}_"):
                out.append(n)
                continue
        if sf:
            stem = sf
...
def _find_finest_node_for_page(structure, page_num: int) -> dict | None:
    ...
    for n in nodes:
        if not isinstance(n, dict):
            continue
        s = n.get("start_index"); e = n.get("end_index", s)
        try:
            si = int(s); ei = int(e)
        ...
        if si >= 0 and si <= page_num <= ei:
            ...
            if isinstance(n.get("nodes"), list):
                ...
                if deeper is not None:
                    best = deeper
                    try:
                        best_span = int(deeper.get("end_index", 0)) - int(deeper.get("start_index", 0))
                    except (TypeError, ValueError):
                        pass
        else:
            if isinstance(n.get("nodes"), list):
...
def _parse_tree_search(raw: str) -> tuple[str, list[str]]:
    """Extract JSON {thinking, node_list} from LLM output."""
...
def main(argv: list[str] | None = None) -> int:
        ...
        json.dump(
            {
                "error": (
                    ...
                    "Pass --tree-json when launching `python -m harness chat ...`."
                )
            },
            sys.stdout,
        )
    ...
    has_findings = any(
        isinstance(n, dict) and n.get("page_findings")
        for n in _walk_nodes(structure)
    )
    if has_findings:
    ...
    system = (
        ...
        "leaf/finest nodes over broad parents."
    )
    ...
    thinking, node_ids = _parse_tree_search(raw)
    ...
    for nid in node_ids:
        ...
        try:
            s = int(node.get("start_index"))
            e = int(node.get("end_index", s))
    ...
    if hit_nodes:
        ...
        if thinking:
            parts.append(f"Search reasoning: {thinking}")
