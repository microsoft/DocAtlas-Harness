...
class BuildSeriesTreeTask:
    ...
    def add_arguments(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--vision", action="store_true",
                            help="Enable vision (page images) for TOC detection.")
        parser.add_argument("--vision-zoom", type=float, default=1.5,
                            help="Render zoom for vision mode.")
        parser.add_argument("--toc-check-pages", type=int, default=20,
                            help="Max pages to scan for TOC.")
        parser.add_argument("--max-pages-per-node", type=int, default=10)
        parser.add_argument("--max-tokens-per-node", type=int, default=20000)
        parser.add_argument("--node-summary", action="store_true", default=True,
                            help="Generate per-node summaries (default on).")
        parser.add_argument("--no-node-summary", dest="node_summary",
                            action="store_false")
        parser.add_argument("--no-think", action="store_true",
                            help="Disable thinking mode for supported models.")
    ...
    def run(args: argparse.Namespace) -> int:
        ...
        trees_dir = Path(args.trees_dir).resolve() if args.trees_dir \
            else output.parent / "trees_cache"
