...
`DocSkills/_common/markdown_reader.py`:
...
Why Docling and not MinerU?
  ...
  - **Docling** is lightweight: `pip install docling`, one CPU-only
...
# ---------------------------------------------------------------------------
# Docling import is lazy — we don't want `python -m harness --help` to crash
...
def _doc_already_built(output_dir: str, doc_stem: str) -> bool:
    """A doc is considered built if it has at least one page md."""
    pattern = os.path.join(output_dir, doc_stem, f"{doc_stem}_page*", "vlm",
                           f"{doc_stem}_page*.md")
...
def _serialize_item_markdown(item, ImageRefMode_cls, doc) -> str:
...
def _build_single(
    ...
    try:
        ...
        opts.generate_page_images = bool(args.page_images)
        ...
        for item, _level in doc.iterate_items():
            ...
            md_frag = _serialize_item_markdown(item, docling_mods["ImageRefMode"], doc)
            if md_frag:
        ...
        # Second pass: write each page's md + images.
        ...
        for page_idx in sorted(page_md_chunks):
            page_dir = os.path.join(
                doc_root, f"{doc_stem}_page{page_idx}", "vlm",
            )
                ...
                for item, rel_name in page_pictures[page_idx]:
                    try:
                        ...
                        if pil_img is not None:
                            pil_img.save(
                                os.path.join(images_dir, rel_name), "PNG",
                            )
                            images_written += 1
                    except Exception:               # noqa: BLE001
...
class BuildMdTask:
    ...
    def add_arguments(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            "--images", action="store_true", default=True,
        ...
        parser.add_argument(
            "--page-images", action="store_true", default=False,
            help="Also save full-page PNG screenshots (not needed for Read skill; "
                 "Read renders pages itself via PyMuPDF).",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--only-sampled", action="store_true",
            help="With --samples-file, skip PDFs not in samples.json.",
        )

    @staticmethod
    def run(args: argparse.Namespace) -> int:
        # Load .env (consistent with other harness commands).
        ...
        if not args.force:
            ...
            for d in docs:
                ...
                if _doc_already_built(output_dir, stem):
        ...
        print(f"  Backend          : docling (lightweight)")
        print(f"  Images           : {'on' if args.images else 'off'} "
              f"(scale={args.images_scale}x)")
        print(f"  Page screenshots : {'on' if args.page_images else 'off'}")
        ...
        log = {
            "config": {
                ...
                "page_images": bool(args.page_images),
                "ocr": bool(args.ocr),
            },
        ...
        for i, d in enumerate(docs, 1):
            ...
            if res["success"]:
                success += 1
                print(f"OK  pages={res['pages_written']}  "
