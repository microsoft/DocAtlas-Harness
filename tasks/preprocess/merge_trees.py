...
``tasks/mmlongbench/io.py::load_series_trees`` and Search/Read's
...
def _load_tree_json(path: str | Path) -> dict:
    ...
    if "structure" not in data:
        # Tolerate "bare list" form: a top-level list of nodes (no doc_name).
        if isinstance(data, list):
            data = {"doc_name": Path(path).stem, "structure": data}
        else:
...
def merge_trees(
    ...
    for idx, path in enumerate(tree_paths):
        ...
        source_pdf = ov.get("source_pdf") or f"{stem}.pdf"
...
class MergeTreesTask:
    ...
    def add_arguments(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            help="With --tree-dir, fnmatch glob to filter file names "
        ...
        parser.add_argument("--doc-description", default=None,
