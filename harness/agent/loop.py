...
class AgentLoop:
    ...
    @staticmethod
    def _trim_input_images(
        items: list[dict], *, max_images: int
    ) -> tuple[list[dict], int]:
    ...
    def _make_image_followup(self, results: list[SkillResult]) -> list[dict]:
        ...
        for r in results:
            ...
            parts.append({
                "type": "input_text",
                "text": f"[Images returned by `{r.skill_name}` follow.]",
            })
            for uri in r.image_uris:
                parts.append({
                    "type": "input_image",
                    "image_url": uri,
        ...
        for turn_num in range(1, self.max_turns + 1):
            ...
            if self.max_input_images > 0:
                input_items, n_trimmed = self._trim_input_images(
                    input_items, max_images=self.max_input_images,
                )
                if n_trimmed:
                    logger.info(
                        "[image-trim] turn %s: dropped %s oldest image(s) (cap=%s)",
                        turn_num, n_trimmed, self.max_input_images,
                    )
            ...
            for fc in response.function_calls:
                ...
                self._cb(
                    "on_tool_result",
                    fc.call_id, fc.name, skill_result.text_output[:2000],
                    tc_elapsed, len(skill_result.image_uris),
                )
            ...
            if self.post_note_hooks is not None:
                ...
                if self.session_store is not None and any(
                    name in self.post_note_hooks.trigger_skills
                    for name, _ in skill_calls_this_turn
                ):
                    try:
                ...
                if archive is not None and archive.modified:
                    ...
                    logger.info(
                        "[post_note] turn %s: %s", turn_num, archive.reason,
                    )
