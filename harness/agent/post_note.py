    ...
    *,
    ...
    while i < n:
        ...
        if isinstance(item, dict) and item.get("type") == "function_call_output":
            ...
            if (
                skill_name in skills_to_archive
                and archive_tag not in current_output
            ):
...
class PostNoteHooks:
    ...
    def _annotate_tree_from_latest_note(self, session_store) -> None:
        ...
        if not entries:
            return
        latest = entries[-1]
        from harness.session.tree import annotate_tree_from_note
    ...
    def maybe_process(
        ...
        triggered = any(
            name in self.trigger_skills for name, _ in last_turn_skill_calls
        )
        if not triggered:
            return None

        # Default behavior comes from the ctor flags.
