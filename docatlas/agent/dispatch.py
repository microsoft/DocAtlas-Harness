...
# Skills intentionally execute as argv-only child processes.
...
#
# Built-in Skills bind their session-level arguments here. Model-selected
# per-call arguments remain governed by each Skill's JSON schema.

SessionArgInjector = Callable[[dict[str, Any], dict[str, Any] | None], list[str]]
...
def _inject_read_session_args(
    session: dict[str, Any], model_args: dict[str, Any] | None = None
) -> list[str]:
    argv: list[str] = []
    ...
    if requested and isinstance(doc_map, dict) and doc_map and entry is None:
        raise ValueError(f"unknown doc_id {requested!r}; available doc_ids: {sorted(doc_map)}")
    if entry is not None:
        pdf = entry.get("pdf_path")
        md_dir = entry.get("markdown_dir")
        did = entry.get("doc_id")
        if not pdf and not (md_dir and did):
            raise ValueError(
                f"doc_map entry for '{requested}' needs pdf_path or markdown_dir + doc_id"
            )
        if pdf:
    ...
    if not pdf and not (md_dir and doc_id):
        raise ValueError("read skill requires a PDF or markdown_dir + doc_id")
    if pdf:
        argv.extend(["--pdf", str(pdf)])
    if md_dir:
        argv.extend(["--markdown-dir", str(md_dir)])
...
def _extract_text_and_images(
    skill_name: str, payload: dict[str, Any]
) -> tuple[str, list[str], list[str]]:
    """Pull model-visible text and labelled image URIs from a skill payload.
    ...
    if isinstance(pages, list):
        ...
        for p in pages:
            ...
            if t:
                ...
                text_chunks.append(f"[Page {page_num} | source={source}]\n{t}")
            img = p.get("page_image")
            if img:
                images.append(img)
                image_labels.append(f"Page {p.get('num', '?')} full-page image")
        if payload.get("mode"):
        ...
        if payload.get("text_is_empty"):
            meta_bits.append(
                "text_is_empty=true (page has no extractable text — request --with-image)"
            )
        if payload.get("auto_vision_fallback"):
            meta_bits.append("auto_vision_fallback=true")
        if payload.get("n_pages_total") is not None:
            meta_bits.append(f"n_pages_total={payload['n_pages_total']}")
        if payload.get("page_count_warning"):
            meta_bits.append(f"page_count_warning={payload['page_count_warning']}")
        if payload.get("session_warning"):
            meta_bits.append(f"session_warning={payload['session_warning']}")

        figure_meta = payload.get("figure_images_meta")
        if isinstance(figure_meta, list) and figure_meta:
            catalog = ["Figure catalog (fetch with Read's figures parameter):"]
            for fig in figure_meta:
                if not isinstance(fig, dict):
                    continue
                size = fig.get("size_px") or []
                size_text = "x".join(str(x) for x in size) if size else "unknown-size"
                caption = str(fig.get("caption", "") or "").strip()
                line = (
                    f"- page={fig.get('page', '?')} ref={fig.get('ref', '?')} "
                    f"size={size_text} bytes={fig.get('bytes', '?')}"
                )
                if caption:
        ...
        figure_errors = payload.get("figure_errors")
        if isinstance(figure_errors, list) and figure_errors:
            text_chunks.append(
                "Figure request errors: " + json.dumps(figure_errors, ensure_ascii=False)
            )
        page_image_errors = payload.get("page_image_errors")
        if isinstance(page_image_errors, list) and page_image_errors:
            text_chunks.append(
                "Page image errors: " + json.dumps(page_image_errors, ensure_ascii=False)
            )
        hint = payload.get("_hint")
        if isinstance(hint, str) and hint.strip():
            text_chunks.append(hint.strip())
        text_blob = "\n\n".join(text_chunks)
        if meta_bits:
        ...
        for fig in extras.get("figure_images") or []:
            ...
            if uri:
                images.append(uri)
                image_labels.append(f"Page {fig.get('page', '?')} figure {fig.get('ref', '?')}")
        return text_blob or "[skill returned no text]", images, image_labels

    # Generic fallback
    text = payload.get("text")
    if isinstance(text, str):
        return text, images, image_labels
    return json.dumps(payload, ensure_ascii=False), images, image_labels
...
class SkillDispatcher:
    ...
    def _subprocess_env(self, skill_name: str) -> dict[str, str]:
        # Do not hand every process unrelated credentials (for example GitHub,
        # AWS, or database tokens). Built-in LLM skills receive only the Azure /
        # OpenAI variables they need; all other additions must be explicit.
        safe_keys = {
            "AZURE_CONFIG_DIR",
            "CURL_CA_BUNDLE",
            "HOME",
    ...
    def call(self, skill_name: str, args: dict[str, Any]) -> SkillResult:
        ...
        if not isinstance(args, dict):
            return SkillResult(
                skill_name=skill_name,
                ok=False,
                text_output="[error] tool arguments must be a JSON object",
            )
        allowed = set((skill.parameters_schema.get("properties") or {}).keys())
        unknown = sorted(str(key) for key in args if key not in allowed)
        if unknown:
            return SkillResult(
                skill_name=skill_name,
                ok=False,
                text_output=f"[error] unknown argument(s): {unknown}; allowed: {sorted(allowed)}",
            )
        ...
        if injector is not None:
            ...
            if skill_name == "read" and (
                self.session_args.get("doc_map") or self.session_args.get("pdf")
            ):
        ...
        text_blob, images, image_labels = _extract_text_and_images(skill_name, payload)
        session_patch = None
        extras = payload.get("_harness_extras") or {}
