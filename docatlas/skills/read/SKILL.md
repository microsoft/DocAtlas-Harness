...
license: MIT
compatibility: Requires Python 3.10-3.13, the complete DocAtlas Skills bundle, and project dependencies installed with uv.
metadata:
...
* **markdown** (preferred): When MinerU per-page markdown is available, you
  get structured headings, tables, and inline figure references. The response
...
Use `zoom` (default 1.0, allowed 0.25-4.0) to control screenshot resolution —
higher values produce larger images and consume more tokens.
...
3. If a figure is too small and was filtered out, use `force_figures: true`
   to bypass the minimum-size filter. Path, file-type, and byte limits still apply.
...
note(found="Revenue was $32.8B in 2018.", evidence=[{"type":"text","source":"Page 47","content":"Revenue was $32.8B in 2018."}])
```
...
* `figure_errors[].reason == "ref_not_found"` — bad figure ref; check `available_refs`.
