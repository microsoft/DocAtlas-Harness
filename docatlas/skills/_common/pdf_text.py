...
_DEFAULT_MAX_TEXT_CHARS_PER_PAGE = 200_000


def _max_text_chars_per_page() -> int:
    try:
        return max(
            1,
            int(
                os.getenv(
                    "HARNESS_MAX_TEXT_CHARS_PER_PAGE",
                    str(_DEFAULT_MAX_TEXT_CHARS_PER_PAGE),
                )
            ),
        )
    except ValueError:
        return _DEFAULT_MAX_TEXT_CHARS_PER_PAGE
    ...
    *missing_pages* so callers can decide how to surface them.
    ...
    for p in page_nums:
        try:
            ...
            max_chars = _max_text_chars_per_page()
            if len(text) > max_chars:
                text = text[:max_chars] + "\n[Page text truncated by safety limit.]"
            if tag:
