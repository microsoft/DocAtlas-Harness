...
license: MIT
compatibility: Requires Python 3.10-3.13, the complete DocAtlas Skills bundle, and a DocAtlas session file.
metadata:
...
Every factual Note call must include evidence copied from the source. Prefer an
exact quote in a `type:"text"` item, and include a physical page reference in
`source`, such as `"Page 33"` or `"Page 12, Table 2"`. Strategy-only and
dead-end checkpoints may omit evidence when they make no factual claim.
...
```bash
uv run --locked python docatlas/skills/note/scripts/run.py --json '{
  ...
  "evidence": [
    {
      ...
      "content": "55% of Republicans and 25% of Democrats say the president can legally pardon himself"
    }
  ]
}'
```
...
|---|---|---|
| `found` | factual notes | 1-3 sentences. Page-anchor anything concrete. |
| `plan` | no | What you intend to do next. Helps reviewers. |
| `evidence` | factual notes | List of `{type, source, content, filename?}`. |

Provide at least one of `found`, `plan`, or `evidence`.
...
uv run --locked python docatlas/skills/note/scripts/run.py --json '{
  ...
  "evidence": [
    {"type": "text", "source": "Page 7", "content": "73% of Republicans and 31% of Democrats say the president has legal authority to pardon himself"}
  ]
}'
```
