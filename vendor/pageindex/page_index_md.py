...
async def get_node_summary(node, summary_token_threshold=200, model=None, md_dir=None):
    node_text = node.get('text')
    num_tokens = count_tokens(node_text, model=model)
    if num_tokens < summary_token_threshold:
        return node_text
    else:
        return await generate_node_summary(node, model=model, md_dir=md_dir)


async def generate_summaries_for_structure_md(structure, summary_token_threshold, model=None, md_dir=None):
    nodes = structure_to_list(structure)
    tasks = [get_node_summary(node, summary_token_threshold=summary_token_threshold, model=model, md_dir=md_dir) for node in nodes]
...
async def md_to_tree(md_path, if_thinning=False, min_token_threshold=None, if_add_node_summary='no', summary_token_threshold=None, model=None, if_add_doc_description='no', if_add_node_text='no', if_add_node_id='yes', md_dir=None):
    with open(md_path, 'r', encoding='utf-8') as f:
        markdown_content = f.read()

    # Resolve md_dir for image extraction (relative image paths in markdown)
    if md_dir is None:
    ...
    if if_add_node_summary == 'yes':
        ...
        tree_structure = await generate_summaries_for_structure_md(tree_structure, summary_token_threshold=summary_token_threshold, model=model, md_dir=md_dir)
